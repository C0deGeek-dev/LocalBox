...
- **Fresh installs now activate complete, verified engine trees.** CUDA release
  planning pairs a server archive with the same-release runtime companion when
  the platform packages it separately, matching OS, architecture, and CUDA
  major. Preview and live output list the same assets and expected sizes;
  downloads extract into a sibling staging directory, and checksum or
  activation failure leaves the working tree and build stamp unchanged.
- Missing `llama-server` errors now name the exact
  `localbox update --mode <mode>` repair. Guided quant rows show size, explicit
  VRAM fit, and the useful part of each catalog note at wide and narrow terminal
  widths. Fresh-profile catalog entries carry structured File/SizeGB/Note data.
- Hugging Face descriptions are decoded, stripped of HTML/script/style content,
  normalized, and length-bounded before catalog persistence. The shipped model
  template no longer contains remote HTML.
- Removed the unreachable proxy log-capture policy from `EnsureProxyConfig`.
  Repointing already tears down a verified old proxy by its socket-table
  listener PID; the regression now asserts that PID is killed exactly once,
  while unverifiable listeners remain protected from termination.
- Stopped writing the inert `SourceType` marker when adding a Hugging Face
  model and removed it from the shipped catalog template. Existing user
  catalogs that still carry it remain loadable and receive the standard
  unknown-field warning.
- Retired the obsolete `LimitTools` catalog field from the shipped model
  template. Existing user catalogs that still carry it remain loadable and now
  receive the standard unknown-field warning.
- **Older AutoBest tunes can be adopted without disguising their history.**
  When no current measurement matches, the guided launcher now offers re-tune,
  explicit adoption, or one-time defaults; `--adopt-tune` provides the same
  fail-closed scripted choice. Adoption updates only the selected entry through
  a temp/backup swap, records its original tuner version and adoption time, and
  applies its runnable settings in the same launch. `models --json` distinguishes
  measured and adopted tunes, while current entries still win and future entries
  remain ineligible.
- **Guided-launcher panels no longer cut actionable text off at the box edge.**
  Rich panels and notice/error screens now wrap long lines, grow to the wrapped
  row count, preserve intentional leading alignment, and show `[…]` when the
  fixed live band is still too short for the whole message.
- **The embedding server's lifecycle belongs to LocalPilot, not to a boot-time
  service.** A machine-level autostart was built and then removed before release:
  a 640 MB model server appearing at login is precisely the unexpected process a
  local-first tool should not install. LocalPilot now starts the server for the
  life of a run and stops it afterwards, and only ever stops a server it started
  — one you brought up with `embed-serve` is yours and survives. `embed-serve`
  and `embed-stop` are unchanged.
...
- **No-think proxy lifecycle in Rust (`localbox-launcher::proxy`).** The
  ...
  reuse-on-match, repoint-on-mismatch (tear down the verified listener and
  restart at the wanted target, never fall back to a silent direct route),
  kill-stale-listener-first before binding
