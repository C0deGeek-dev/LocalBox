...
/// Default no-think proxy port when the merged catalog does not configure one.
pub const DEFAULT_PROXY_PORT: u16 = 11_435;
...
pub struct LaunchRequest {
    ...
    pub bypass: bool,
    /// An explicitly requested no-think proxy listen port. `None` means the
    /// caller expressed no preference, so the configured `NoThinkProxyPort`
    /// decides and [`DEFAULT_PROXY_PORT`] is the floor. A sentinel value would
    /// make "the user asked for 11435" indistinguishable from "nobody asked".
...
pub fn plan_launch(
    ...
    let proxy_port = request
        .proxy_port
        .or_else(|| launcher.configured_no_think_proxy_port())
...
mod tests {
    ...
    fn launcher(dir: &std::path::Path) -> LlamaLauncher {
        launcher_with_proxy_port(dir, None)
    }

    fn launcher_with_proxy_port(dir: &std::path::Path, proxy_port: Option<u16>) -> LlamaLauncher {
        ...
        if let Some(proxy_port) = proxy_port {
            settings.insert("NoThinkProxyPort".to_string(), proxy_port.into());
        }
    ...
    fn planning_uses_the_configured_proxy_port_or_the_default() {
        ...
        let request = LaunchRequest::new("q36apex", "64k", Mode::Turboquant);
        let plan = plan_launch(&configured, &request).expect("configured plan");
        assert_eq!(plan.proxy.listen_port, 11_500);
        assert!(plan.base_url.ends_with(":11500"));

        let fallback = launcher(dir.path());
        let plan = plan_launch(&fallback, &request).expect("fallback plan");
        assert_eq!(plan.proxy.listen_port, DEFAULT_PROXY_PORT);
        assert!(plan.base_url.ends_with(":11435"));
    }

    /// An explicit request wins over the configured value, including when it
    /// happens to equal the default. A sentinel comparison would silently
    /// hand this launch the catalog's port instead.
    #[test]
    fn an_explicit_request_port_outranks_the_configured_one() {
        let dir = tempfile::tempdir().unwrap();
        let configured = launcher_with_proxy_port(dir.path(), Some(11_500));
        let mut request = LaunchRequest::new("q36apex", "64k", Mode::Turboquant);
        request.proxy_port = Some(DEFAULT_PROXY_PORT);
        let plan = plan_launch(&configured, &request).expect("explicit plan");
        assert_eq!(plan.proxy.listen_port, DEFAULT_PROXY_PORT);
    }
