...
impl LlamaLauncher {
    ...
    /// Configured no-think proxy port, when present in the merged catalog.
    #[must_use]
    pub fn configured_no_think_proxy_port(&self) -> Option<u16> {
        self.catalog.no_think_proxy_port()
    }
    ...
    pub fn model_download_targets(
        ...
        let mut targets: Vec<ModelDownload> = shard_files_from_primary(&gguf_file)
            .into_iter()
            .map(|file| {
...
mod tests {
    ...
    fn install_roots_and_binary_resolution_are_per_mode() {
        ...
        for mode in [
            ...
            assert!(
                err.to_string()
                    .contains(&format!("localbox update --mode {repair_mode}")),
                "{err}"
            );
        }
