...
3. Brings up the no-think proxy on `127.0.0.1:11435` in front of
   ...
   A verified proxy already serving the right target and exposure posture is
   reused. On a target or posture mismatch, its listener PID is killed once
   before LocalBox starts the replacement; an unverifiable listener is refused
   rather than killed.
...
Nothing here starts at boot, and LocalBox installs no service. That is
deliberate: a 640 MB model server appearing at login is exactly the kind of
unexpected process a local-first tool should not put on someone's machine.

**LocalPilot starts it for the life of a run** and stops it afterwards, when the
project has an embedding endpoint configured. It starts nothing for a project
that never asked for embeddings, and nothing for a command that is not a session
— `--version` does not launch a model server. See LocalPilot's
`docs/localmind-integration.md`.

`embed-serve` and `embed-stop` remain the manual controls, and they win:
LocalPilot only stops a server **it started**. A server you brought up yourself
is yours, and a LocalPilot run ending leaves it exactly where it was.
