...
Install the required `llama-server` engine with
`localbox update --mode <mode>`; missing-binary errors print that exact repair.
`--check` previews the same asset set and expected download size without
changing anything. GGUF weights download from Hugging Face on first launch.

## Per platform

- **Windows** — CUDA or CPU `llama-server` builds download automatically,
  matched to your driver's CUDA major version. A CUDA server and its separately
  packaged `cudart` archive must come from the same release and match the OS,
  architecture, and CUDA major; a missing companion aborts with a pin/release
  remedy instead of installing only half the runtime.
...
All selected engine assets are downloaded and digest-verified in a sibling
staging directory. LocalBox writes the build stamp there only after the staged
tree contains `llama-server`, then swaps the complete tree into place. A failed
server/runtime download, checksum, extraction, or activation leaves the
previous install and stamp intact.
