...
"q36plus": {
  ...
  "Quants": {
    "q4kp":  { "File": "model-Q4_K_P.gguf", "SizeGB": 18.1,
                "Note": "Higher quality for roomy GPUs" },
    "iq4xs": { "File": "model-IQ4_XS.gguf", "SizeGB": 14.2,
                "Note": "More context headroom" }
  },
...
Any description supplied by the Hub is treated as untrusted display input:
entities are decoded, HTML plus script/style bodies are removed, whitespace is
normalized, and the result is length-bounded before it reaches the catalog or
`localbox info`. Raw model-card markup is never rendered or executed.
...
Per-quant tradeoffs come from `SizeGB` (drives the fit badge) and `Note`
(human-readable quality/use-case context) on each structured `Quants` entry.
The guided row keeps the quant, size, and explicit fit verdict visible, removes
note fragments that only repeat the quant/size, and ellipsizes the remaining
note on narrow terminals. Legacy string quants plus `QuantSizesGB`/
`QuantNotes` still migrate without overwriting user values, but new entries and
the shipped fresh-profile catalog use `{File,SizeGB,Note}` throughout.
