...
the live band stays in your native scrollback. Panel and notice text wraps at
the box edge, and the box grows to the wrapped height. If the live band is too
short for the complete message, `[…]` marks the clipped final row instead of
silently hiding the rest. The plain fallback keeps errors to one bounded,
